...
void ProcessingPresenter::Test()
{
    ...
    //    -77provider->Load("test_frame.dng", FileFormat::DNG, *_image.get(),
    //    *poolAllocator);
    //    //    provider->Load("M11-1526.VB.mlv", FileFormat::MLV, *_image.get(),
    //    *poolAllocator); OC_LOG_INFO("Loading finished");
    ...
    //    std::string frameTimeLog = "Frame loading time: " +
    //    std::to_string(frameTime) + "ms"; OC_LOG_INFO(frameTimeLog);
    ...
    //    unsigned short*  redArray = static_cast<unsigned
    //    short*>(_image->RedChannel()); unsigned short*  greenArray =
    //    static_cast<unsigned short*>(_image->GreenChannel()); unsigned short*
    //    blueArray = static_cast<unsigned short*>(_image->BlueChannel());
    ...
    //    //lodepng::encode("output.png", interleavedArray, _image->Width(),
    //    _image->Height(), LodePNGColorType::LCT_RGB, 8);

    //    unsigned char dummydata[] = "This is a dummy data to just test if the
    //    dump feature works or not."; OC::Image::RawDump::Dump("DummyDump.dat",
    //    dummydata, strlen((const char*)dummydata));
    //    OC::Image::RawDump::Dump("InterleavedArray.dat", interleavedArray,
    //    dataLength * 3); OC_LOG_INFO("Conversion finished");

    //    _view->SetThumbnail(_image->Width(), _image->Height(),
    //    interleavedArray);
...
void ProcessingPresenter::Show()
{
    ...
    //    lodepng::encode("color.png", interleavedArray, _image->Width(),
    //    _image->Height(), LodePNGColorType::LCT_RGB, 8);
